...
If your transcript covers multiple talks, you can use the `summarise_full_meeting()` function to generate a summary for each talk in the transcript.
...
The output of the `summarise_full_meeting()` function is a `summary_tree`, i.e., a list containing the summary and some other information about each talk.
...
The package exposes the function `set_prompts()` function to edit the prompts.

Be careful, changing the prompt will change the output drastically. The prompts use the `glue` package syntax to automatically insert some relevant variables in the prompts, which are needed for many of the package feature to work. `set_promts(force = TRUE)` can be used to restore all prompts to their default values.
